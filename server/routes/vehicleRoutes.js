    ...
    db.get(
        ...
        (err, existing_vehicle) => {
            ...
            db.get(
                ...
                (err, user) => {
                    if (err) {
                        console.error('[ERROR] User lookup error:', err);
                        return res.status(500).json({ error: err.message });
                    }
                    ...
                    db.run(
                        ...
                        function(err) {
                            ...
                            if (parking_slots && parking_slots.length > 0) {
                                updateParkingSlots(parking_slots);
                            }
...
router.post('/exit', (req, res) => {
    ...
    db.get(
        ...
        (err, vehicle) => {
            if (err) {
                console.error('[ERROR] Vehicle lookup error:', err);
                return res.status(500).json({ error: err.message });
            }
...
function processPayment(vehicle_id, user_id, rfid_uid, amount, callback) {
    ...
    if (!user_id) {
        ...
        callback('pending', transaction_id);
        return;
    }
    ...
    db.get(
        ...
        (err, user) => {
            if (err) {
                console.error('[ERROR] User lookup error:', err);
                callback('failed', transaction_id);
                return;
            }
            ...
            if (user && user.account_balance >= amount) {
                ...
                db.run(
                    ...
                    (err) => {
                        if (err) {
                            console.error('[ERROR] Balance update error:', err);
                            callback('failed', transaction_id);
                            return;
                        }
                        
                        // Record transaction
                        recordTransaction(transaction_id, user_id, vehicle_id, amount, 'completed');
                        console.log(`[PAYMENT] Completed - Balance deducted ${amount} VND`);
                        callback('completed', transaction_id);
                    }
                );
            } else {
...
function recordTransaction(transaction_id, user_id, vehicle_id, amount, status) {
    db.run(
        ...
        (err) => {
            if (err) {
                console.error('[ERROR] Transaction record error:', err);
            }
        }
    );
}
...
function updateParkingSlots(slots) {
    ...
    for (let i = 0; i < slots.length && i < 6; i++) {
        ...
        db.run(
            ...
            (err) => {
                if (err) {
                    console.error(`[ERROR] Parking slot update error:`, err);
                }
            }
        );
    }
